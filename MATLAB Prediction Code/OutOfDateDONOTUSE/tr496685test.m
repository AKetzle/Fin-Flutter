...
freq_h = 209.51;
r_alpha = sqrt(0.065641);
r_h = 0;
b = 1.25; %ft
c = 0;
rho = 0.076474252; % lbm/ft^3
m = 0.92667; % lbm
a = 0;
x_alpha = 0;
...
a = location of axis of rotation of fin from leading edge
x_alpha = distance between a 
c = chord length
* beta inputs are omitted because this assumes a fin without ailerons
%}
r_beta = sqrt(0.0012);
freq_beta = sqrt((3/2)*freq_h^2);
x_beta = 0;
g_alpha = 0;
g_beta = 0;
g_h = 0;
...
% syms k X; % k is the reduced frequency, X is quantity that makes everything work
% 
% J_1 = besselj(1,k);
% J_0 = besselj(0,k);
% Y_0 = bessely(0,k);
...
for i=1:iterations
    ...
    [Xi, ki] = vpasolve([x_realeq, x_imageq],[X, k],[X0, 100; k0, inf]);
    Xdiff = Xi - X0;
    kdiff = ki - k0;
